...
function TaskCardComponent({ item }: TaskCardProps) {
  const { model, viewTask, editTask, deleteTask } = item
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "bg-card border-border hover:border-border relative flex min-w-0 w-full touch-none items-start gap-2 rounded-xl border p-3 shadow-sm transition-[box-shadow,border-color] hover:shadow-md",
        isDragging &&
